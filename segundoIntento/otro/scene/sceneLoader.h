...
bool newPopulate(Camera& camera, list<Figure*>& listFigures,
	...
	string line, type;
	stringstream ss;
	float px, py, pz, lx, ly, lz, ux, uy, uz, fx, fy, fz;
	int w, h;
	float rd, gd, bd, rs, gs, bs, rr, gr, br;
	float nx, ny, nz, d, f;
	...
	while (getline(file, line)) {
		...
		if(type == "Camara") {
			ss >> parentesis1 >> px >> coma >> py >> coma >> pz >> parentesis2;
			ss >> parentesis1 >> lx >> coma >> ly >> coma >> lz >> parentesis2;
			ss >> parentesis1 >> ux >> coma >> uy >> coma >> uz >> parentesis2;
			ss >> parentesis1 >> fx >> coma >> fy >> coma >> fz >> parentesis2;
			ss >> w >> h;
			camera = Camera(Point(px, py, pz), Direction(lx, ly, lz), Direction(ux, uy, uz), Direction(fx, fy, fz), w, h);
			cameraDefined = true;
		}

		if(type == "Luz") {
			ss >> parentesis1 >> px >> coma >> py >> coma >> pz >> parentesis2;
			ss >> parentesis1 >> rd >> coma >> gd >> coma >> bd >> parentesis2;
			lightSources.push_back(LightSource(Point(px, py, pz), Color(rd, gd, bd)));
			lightDefined = true;
		}
		...
		if(type == "Plano") {
			ss >> parentesis1 >> nx >> coma >> ny >> coma >> nz >> parentesis2;
			ss >> d;
			ss >> parentesis1 >> rd >> coma >> gd >> coma >> bd >> parentesis2;
			ss >> parentesis1 >> rs >> coma >> gs >> coma >> bs >> parentesis2;
			ss >> parentesis1 >> rr >> coma >> gr >> coma >> br >> parentesis2;
			ss >> parentesis1 >> px >> coma >> py >> coma >> pz >> parentesis2;
			ss >> f;
			listFigures.push_back(new Plane(idFig, Direction(nx, ny, nz), d, Color(rd, gd, bd), Color(rs, gs, bs), Color(rr, gr, br), Color(px, py, pz), f));
			idFig++;
			figureDefined = true;
		}
		...
		if(type == "Esfera") {
			...
			ss >> parentesis1 >> px >> coma >> py >> coma >> pz >> parentesis2;
			ss >> f;
			listFigures.push_back(new Sphere(idFig, Point(nx, ny, nz), d, Color(rd, gd, bd), Color(rs, gs, bs), Color(rr, gr, br), Color(px, py, pz), f));
			idFig++;
			figureDefined = true;
		}
	}
